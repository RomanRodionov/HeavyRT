...
void BVH2_LRFT::IntersectAllPrimitivesInLeaf(const float3 ray_pos, const float3 ray_dir,
...
CRT_Hit BVH2_LRFT::RayQuery_NearestHit(float4 posAndNear, float4 dirAndFar)
{
  ...
  int top = 0;
  uint32_t leftNodeOffset  = m_allNodes[bvhOffset].leftOffset; 
  uint32_t rightNodeOffset = m_allNodes[bvhOffset].escapeIndex; 
  ...
  while (top >= 0 && !(stopOnFirstHit && hit.primId != uint32_t(-1)))
  {
    while (top >= 0 && ((leftNodeOffset & LEAF_BIT) == 0))
    {
      ...
      const BVHNode node0 = m_allNodes[bvhOffset + leftNodeOffset];
      const BVHNode node1 = m_allNodes[bvhOffset + rightNodeOffset];
      ...
      {
        leftNodeOffset  = (tm0.x <= tm1.x) ? node0.leftOffset  : node1.leftOffset;   
        rightNodeOffset = (tm0.x <= tm1.x) ? node0.escapeIndex : node1.escapeIndex;
        stack[top+0]    = (tm0.x <= tm1.x) ? node1.leftOffset  : node0.leftOffset; 
        stack[top+1]    = (tm0.x <= tm1.x) ? node1.escapeIndex : node0.escapeIndex; 
        top+=2;
        #ifdef ENABLE_METRICS
        m_stats.SOC++;
      ...
      {
        top-=2;
        leftNodeOffset  = stack[std::max(top+0,0)];
        rightNodeOffset = stack[std::max(top+1,0)];
        #ifdef ENABLE_METRICS
        m_stats.SOC++;
        m_stats.SBL+=sizeof(uint32_t); 
        #endif
    ...
    if (top >= 0 && leftNodeOffset < 0xFFFFFFFE && bvhOffset != m_tlasOffset)  // leaf node of BLAS, intersect triangles
    {
    ...
    else if (top >= 0 && bvhOffset == m_tlasOffset)                            // leaf node of BLAS, intersect BLAS next
    {
      ...
      leftNodeOffset   = m_allNodes[bvhOffset].leftOffset; 
      rightNodeOffset  = m_allNodes[bvhOffset].escapeIndex;       
    ...
    if(top >= 0 && needStackPop) // continue BVH traversal, except the cases when we have just enter from TLAS to BLASS (see upper code)
    {
      top-=2;
      if(top >= 0)               // don't remove this check, or it won't work on mobile!
      {
        leftNodeOffset  = stack[std::max(top+0,0)];
        rightNodeOffset = stack[std::max(top+1,0)];
      }
...
bool BVH2_LRFT::RayQuery_AnyHit(float4 posAndNear, float4 dirAndFar)
{
  dirAndFar.w *= -1.0f;

...
static inline BVHNode BVHNodeFromHalfNode(BVHNode16 a_node)
{
  BVHNode res;
  res.boxMin.x = float(a_node.minXYZmaxX.x);
  res.boxMin.y = float(a_node.minXYZmaxX.y);
  res.boxMin.z = float(a_node.minXYZmaxX.z);
  res.boxMax.x = float(a_node.minXYZmaxX.w);
  res.boxMax.y = float(a_node.maxYZ.x);
  res.boxMax.z = float(a_node.maxYZ.y);
  res.leftOffset = a_node.leftOffset;
  return res;
}

void BVH2_LOFT_16::IntersectAllPrimitivesInLeaf(const float3 ray_pos, const float3 ray_dir,
...
CRT_Hit BVH2_LOFT_16::RayQuery_NearestHit(float4 posAndNear, float4 dirAndFar)
{
  ...
  while (top >= 0 && !(stopOnFirstHit && hit.primId != uint32_t(-1)))
  {
    while (top >= 0 && ((leftNodeOffset & LEAF_BIT) == 0))
    {
      ...
      const BVHNode node0 = BVHNodeFromHalfNode(m_allNodes16[bvhOffset + leftNodeOffset + 0]);
      const BVHNode node1 = BVHNodeFromHalfNode(m_allNodes16[bvhOffset + leftNodeOffset + 1]);
...
bool BVH2_LOFT_16::RayQuery_AnyHit(float4 posAndNear, float4 dirAndFar)
{
  dirAndFar.w *= -1.0f;

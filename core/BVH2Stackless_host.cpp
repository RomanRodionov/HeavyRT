...
size_t BVH2Stackless::AppendTreeData(const std::vector<cbvh::BVHNode>& a_nodes, const std::vector<uint32_t>& a_indices, 
...
void BVH2Stackless::CommitScene(BuildQuality a_qualityLevel)
{
  ...
  m_allNodes.insert(m_allNodes.end(), m_nodesTLAS.begin(), m_nodesTLAS.end());
...
uint32_t BVH2Stackless::AddInstance(uint32_t a_geomId, const float4x4 &a_matrix)
{
...
void BVH2Stackless::UpdateInstance(uint32_t a_instanceId, const float4x4 &a_matrix)
{
  std::cout << "[BVH2Stackless::UpdateInstance]: "
            << "not implemeted!" << std::endl;
}

ISceneObject *MakeBVH2Stackless(const char *a_implName, const char* a_buildName) { return new BVH2Stackless(a_buildName); }
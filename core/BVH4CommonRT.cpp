...
void BVH4CommonRT::IntersectAllPrimitivesInLeaf(const float3 ray_pos, const float3 ray_dir, 
...
void BVH4CommonRT::BVH4Traverse(const float3 ray_pos, const float3 ray_dir, float tNear, uint32_t instId, uint32_t geomId, bool stopOnFirstHit, CRT_Hit* pHit)
{
  ...
  while (top >= 0 && !(stopOnFirstHit && pHit->primId != uint32_t(-1)))
  {
    ...
    if ((leftNodeOffset & LEAF_BIT) == 0)
    {
      ...
      uint4 children = uint4(node0.leftOffset, node1.leftOffset, node2.leftOffset, node3.leftOffset);
...
CRT_Hit BVH4CommonRT::RayQuery_NearestHit(float4 posAndNear, float4 dirAndFar)
{
...
bool BVH4CommonRT::RayQuery_AnyHit(float4 posAndNear, float4 dirAndFar)
{
  dirAndFar.w *= -1.0f;

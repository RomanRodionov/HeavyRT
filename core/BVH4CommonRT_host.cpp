...
#include "BVH4CommonRT.h"

constexpr size_t reserveSize = 1000;

void BVH4CommonRT::ClearGeom()
{
...
void BVH4CommonRT::AppendTreeData(const std::vector<BVHNode>& a_nodes, const std::vector<uint32_t>& a_indices, 
...
uint32_t BVH4CommonRT::AddGeom_Triangles3f(const float* a_vpos3f, size_t a_vertNumber, const uint32_t* a_triIndices, size_t a_indNumber, BuildQuality a_qualityLevel, size_t vByteStride)
{
  ...
  //cbvh2::BuilderPresets presets = {cbvh2::BVH4_LEFT_OFFSET, cbvh2::BVH_CONSTRUCT_FAST, 2};
...
uint32_t BVH4CommonRT::AddInstance(uint32_t a_geomId, const float4x4& a_matrix)
{
...
void BVH4CommonRT::UpdateInstance(uint32_t a_instanceId, const float4x4& a_matrix)
{
  std::cout << "[BVH4CommonRT::UpdateInstance]: " << "not implemeted!" << std::endl;
}

ISceneObject* MakeBVH4CommonRT(const char* a_implName, const char* a_buildName) { return new BVH4CommonRT(a_buildName); }

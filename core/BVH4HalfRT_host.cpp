...
void BVH4HalfRT::ClearGeom()
{
  ...
  m_allNodes16.reserve(std::max<size_t>(100000, m_allNodes16.capacity()));
  m_allNodes16.resize(0);
...
uint32_t BVH4HalfRT::AddGeom_Triangles3f(const float* a_vpos3f, size_t a_vertNumber, const uint32_t* a_triIndices, size_t a_indNumber, BuildQuality a_qualityLevel, size_t vByteStride)
{
  ...
  const size_t oldBvhSize = m_allNodes16.size(); // both Nodes and Intervals always have same size, we know that
  m_bvhOffsets.push_back(oldBvhSize); 

  // Build BVH for each geom and append it to big buffer
  //
  //cbvh2::BuilderPresets presets = {cbvh2::BVH4_LEFT_OFFSET, cbvh2::BVH_CONSTRUCT_QUALITY, 2};
  ...
  presets.format = cbvh2::BVH4_LEFT_OFFSET;
  auto bvhDataC  = cbvh2::BuildBVHHalf((const float*)(m_vertPos.data() + oldSizeVert), a_vertNumber, 16, a_triIndices, a_indNumber, presets);
 
  m_allNodes16.insert(m_allNodes16.end(), bvhDataC.nodes.begin(), bvhDataC.nodes.end());
...
void BVH4HalfRT::UpdateGeom_Triangles3f(uint32_t a_geomId, const float* a_vpos3f, size_t a_vertNumber, const uint32_t* a_triIndices, size_t a_indNumber, BuildQuality a_qualityLevel, size_t vByteStride)
{
  std::cout << "[BVH4HalfRT::UpdateGeom_Triangles3f]: " << "not implemeted!" << std::endl;
}

void BVH4HalfRT::ClearScene()
{
...
void BVH4HalfRT::CommitScene(BuildQuality a_qualityLevel)
{
  ...
  //  
  m_stats.clear();
  m_stats.bvhTotalSize  = m_allNodes16.size()*sizeof(uint4) + m_nodesTLAS.size()*sizeof(BVHNode);
  m_stats.geomTotalSize = m_vertPos.size()*sizeof(float4)  + m_indices.size()*sizeof(uint32_t);
} 

uint32_t BVH4HalfRT::AddInstance(uint32_t a_geomId, const float4x4& a_matrix)
{
...
void BVH4HalfRT::UpdateInstance(uint32_t a_instanceId, const float4x4& a_matrix)
{
  std::cout << "[BVH4HalfRT::UpdateInstance]: " << "not implemeted!" << std::endl;
}

ISceneObject* MakeBVH4HalfRT(const char* a_implName, const char* a_buildName) { return new BVH4HalfRT(a_buildName); }

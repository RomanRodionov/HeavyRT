...
RayBoxIntersection(float3 rayOrigin, float3 rayDirInv, float3 boxMin, float3 boxMax)
{
...
static inline float2 unpackFloat2x16_3(uint32_t
                                       v)
{
  LiteMath::half xy[2];
  memcpy(xy, &v,
...
static inline BVHNode UnpackBVHNode_3(uint4 data)
{
  const float2 XY = float2(unpackFloat2x16_3(data.x));
  const float2 ZX = float2(unpackFloat2x16_3(data.y));
  const float2 YZ = float2(unpackFloat2x16_3(data.z));
  BVHNode node;
  node.boxMin.x = XY.x;
  node.boxMin.y = XY.y;
  node.boxMin.z = ZX.x;
...
bool Sht4NodesHalfRT::RayQuery_AnyHit(float4 posAndNear, float4 dirAndFar)
{
  dirAndFar.w *= -1.0f;
...
uint32_t Sht4NodesHalfRT::CountTrailingZeros(uint64_t bitTrail)
{
...
void Sht4NodesHalfRT::IntersectAllPrimitivesInLeaf(const float3 ray_pos, const float3 ray_dir,
                                                   ...
                                                   uint32_t a_count, CRT_Hit *pHit)
{
  ...
  for(uint32_t triId = a_start; triId < a_start + a_count; triId++)
  {
    ...
    if(v >= -1e-6f && u >= -1e-6f && (u + v <= 1.0f + 1e-6f) && t > tNear && t <
                                                                             pHit->t) // if (v > -1e-6f && u > -1e-6f && (u + v < 1.0f+1e-6f) && t > tMin && t < hit.t)
    {
      pHit->t = t;
      pHit->primId = triId;
      pHit->instId = instId;
...
CRT_Hit Sht4NodesHalfRT::RayQuery_NearestHit(float4 posAndNear, float4 dirAndFar)
{
  ...
  do
  {
    ...
    do
    {
      ...
      const float2 boxHit = RayBoxIntersection2(to_float3(posAndNear), rayDirInv, currNode.boxMin,
                                                currNode.boxMax);
      const bool intersects = (boxHit.x <= boxHit.y) && (boxHit.y > posAndNear.w) && (boxHit.x <
                                                                                      hit.t); // (tmin <= tmax) && (tmax > 0.f) && (tmin < curr_t)
      ...
      travFlags = (currNode.leftOffset & LEAF_BIT) | uint32_t(
              intersects); // travFlags  = (((currNode.leftOffset & LEAF_BIT) == 0) ? 0 : LEAF_BIT) | (intersects ? 1 : 0);
    ...
    if(isLeafAndIntersect(travFlags))
    {
      ...
      const float3 ray_dir = matmul3x3(m_instMatricesInv[instId], to_float3(
              dirAndFar)); // DON'T NORMALIZE IT !!!! When we transform to local space of node, ray_dir must be unnormalized!!!
...
void
Sht4NodesHalfRT::Traverse(const float3 rayPos, const float3 ray_dir, float tMin, uint32_t instId,
                          uint32_t geomId,
                          CRT_Hit *hit)
{
  ...
  while(true)
  {
...
#endif
      const BVHNode node0 = UnpackBVHNode_3(m_allNodes[bvhOffset + leftNodeOffset + 0]);
      const BVHNode node1 = UnpackBVHNode_3(m_allNodes[bvhOffset + leftNodeOffset + 1]);
      const BVHNode node2 = UnpackBVHNode_3(m_allNodes[bvhOffset + leftNodeOffset + 2]);
      const BVHNode node3 = UnpackBVHNode_3(m_allNodes[bvhOffset + leftNodeOffset + 3]);

      uint4 leftOffsets = uint4(node0.leftOffset, node1.leftOffset, node2.leftOffset,
                                node3.leftOffset);
    ...
    leftNodeOffset = hashTable[hOffset +
                               (nodeKey + displacementTables[dOffset + (nodeKey & dSize)]) % hSize];
  }
}


void Sht4NodesHalfRT::UpdateInstance(uint32_t
                                     a_instanceId,
                                     const float4x4 &a_matrix
)
{
  std::cout << "[Sht4NodesHalfRT::UpdateInstance]: "
            << "not implemeted!" <<
            std::endl;
}

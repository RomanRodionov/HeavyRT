...
bool ShtRT::RayQuery_AnyHit(float4 posAndNear, float4 dirAndFar)
{
...
uint32_t ShtRT::CountTrailingZeros(uint32_t bitTrail)
{
  uint32_t bits = 0, x = bitTrail;
...
void ShtRT::IntersectAllPrimitivesInLeaf(const float3 ray_pos, const float3 ray_dir, float tNear, uint32_t instId,
...
CRT_Hit ShtRT::RayQuery_NearestHit(float4 posAndNear, float4 dirAndFar)
{
  #ifdef ENABLE_METRICS
  ResetVarLC();
  m_stats.raysNumber++;
...
void ShtRT::Traverse(const float3 rayPos, const float3 ray_dir, float tMin, uint32_t instId, uint32_t geomId, CRT_Hit *hit)
{
  ...
  uint32_t nodeKey = 1; //key for root; for node with key=k its left_key=2 * k and right_key=2 * k + 1
  uint32_t bitTrail = 0;

...
void ShtRT::AppendTreeData(const std::vector<BVHNode> &a_nodes, const std::vector<uint32_t> &a_indices,
...
uint32_t
ShtRT::AddGeom_Triangles3f(const float *a_vpos3f, size_t a_vertNumber, const uint32_t *a_triIndices, size_t a_indNumber,
...
void ShtRT::UpdateGeom_Triangles3f(uint32_t a_geomId, const float *a_vpos3f, size_t a_vertNumber,
                                   const uint32_t *a_triIndices, size_t a_indNumber, BuildQuality a_qualityLevel,
                                   size_t vByteStride)
{
  std::cout << "[BVH2ShtRT::UpdateGeom_Triangles3f]: "
            << "not implemeted!" << std::endl;
}

void ShtRT::ClearScene()
{
...
uint32_t ShtRT::AddInstance(uint32_t a_geomId, const float4x4 &a_matrix)
{
...
void ShtRT::UpdateInstance(uint32_t a_instanceId, const float4x4 &a_matrix)
{
  std::cout << "[ShtRT::UpdateInstance]: "
            << "not implemeted!" << std::endl;
}

ISceneObject *MakeShtRT(const char *a_implName, const char *a_buildName)
{ return new ShtRT(a_buildName); }

cbvh::BVHPresets ShtRT::GetShtPresetsFromName(const std::string &a_buildName, BuildQuality a_qualityLevel)
{
...
void
ShtRT::BuildBVHWithUncles(std::vector<BVHNode> &bvhNodes, uint32_t node, uint32_t uncle, uint32_t brother)
{
...
void ShtRT::InitHashTables()
{
  ...
  for (uint32_t objectId = 0; objectId < objNumber; ++objectId) {
    ...
    std::map<uint32_t, uint32_t> keysAddressMap;
...
void ShtRT::InitDisplacementMap(std::map<uint32_t, std::vector<uint32_t>> &displacementMap, std::map<uint32_t, uint32_t> &keys, uint32_t dSize) {
...
bool ShtRT::CanPlaceValuesToHashtable(std::vector<uint32_t> &keys, uint32_t dispValue, uint32_t hSize, uint32_t hOffset) {
  for (auto &key: keys) {
...
void ShtRT::InitHashTable(std::vector<std::pair<uint32_t, std::vector<uint32_t>>> &displacementVector, std::map<uint32_t, uint32_t> &keysAddressMap, uint32_t objectId) {
...
void ShtRT::CountDisplacementSize(uint32_t counter, uint32_t objectId) {
  auto v = counter / 2;
  v -= 1;
  v |= v >> 1;
  v |= v >> 2;
...
void ShtRT::CountHashtableSize(uint32_t counter, uint32_t objectId) {
  auto v = counter * 2;
...
void ShtRT::CountNodesKeys(uint32_t offset, const uint32_t currentNode, const uint32_t nodeKey, uint32_t &counter, std::map<uint32_t, uint32_t> &keysAddressMap) {

...
cmesh::SimpleMesh2 cmesh::CreateQuad(const int a_sizeX, const int a_sizeY, const float a_size)
{
  ...
  cmesh::SimpleMesh2 res(vertNum, quadsNum*2*3);
...
cmesh::SimpleMesh2 cmesh::LoadMeshFromVSGF2(AAssetManager* mgr, const char* a_fileName)
{
  AAsset* asset = AAssetManager_open(mgr, a_fileName, AASSET_MODE_STREAMING);
  if (!asset)
  {
//    LOG("Could not load mesh from \"%s\"!", a_fileName);
    return SimpleMesh2();
  }
  assert(asset);
  ...
  SimpleMesh2 res(vsgf_header.verticesNum, vsgf_header.indicesNum);
...
cmesh::SimpleMesh2 cmesh::LoadMeshFromVSGF2(const char* a_fileName)
{
  ...
  SimpleMesh2 res(header.verticesNum, header.indicesNum);
  ...
  {
    for(auto& v : res.vNorm4f) // #TODO: calc at flat normals in this case
      v = float4(0,0,0,0);
  }
...
void cmesh::SaveMeshToVSGF(const char* a_fileName, const SimpleMesh2& a_mesh)
{
...
float cmesh::SimpleMesh2::GetAvgTriArea() const
{
...
float cmesh::SimpleMesh2::GetAvgTriPerimeter() const
{
  long double res = 0.0f;
  for(size_t i = 0; i < TrianglesNum(); i++)
  {
...
void cmesh::SimpleMesh2::ApplyMatrix(const LiteMath::float4x4& m)
{

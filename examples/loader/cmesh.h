...
namespace cmesh
{
  ...
  struct SimpleMesh2
  {
    ...
    SimpleMesh2(){}
    SimpleMesh2(int a_vertNum, int a_indNum) 
    { 
      Resize(a_vertNum, a_indNum); 
    }
...
#if defined(__ANDROID__)
  SimpleMesh2 LoadMeshFromVSGF2(AAssetManager* mgr, const char* a_fileName);
#else
  SimpleMesh2 LoadMeshFromVSGF2(const char* a_fileName);
#endif
  void        SaveMeshToVSGF  (const char* a_fileName, const SimpleMesh2& a_mesh);
  SimpleMesh2 LoadMeshViaAssimp(const char* a_fileName);

  SimpleMesh2 CreateQuad(const int a_sizeX, const int a_sizeY, const float a_size);

  void WeldVertices(SimpleMesh2& mesh, int indexNum);
};
